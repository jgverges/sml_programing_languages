...
fun dates_in_month (dateList : (int*int*int)list , month : int)=
    ...
    fun search_month(count:(int*int*int)list, dateList:(int*int*int)list)=
        if null dateList
        then tl count
        else search_month(if #2(hd dateList)= month 
                            then append_dates(count,[(hd dateList)]) 
                            else count, 
                         tl dateList)
    in
...
fun dates_in_months(dateList : (int*int*int)list , months : int list)=
    ...
    fun search_by_month(count:(int*int*int)list, months)=
        if null months
        then tl count
        else search_by_month(append_dates(count, dates_in_month(dateList, hd months)),
                             tl months)
    in
...
fun number_before_reaching_sum (total:int, numbers:int list)=
 ...
 fun what_month(day_of_year: int)=
    ...
    then
       number_before_reaching_sum(day_of_year,[0,31,28,31,30,31,30,31,31,30,31,30,31])
    else 0   
